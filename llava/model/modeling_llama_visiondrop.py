...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    """
    # print(q.device, k.device, cos.device, position_ids.device)
    # exit()
...
class LlamaModel(LlamaPreTrainedModel):
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if rank_layer in self.layer_list:
                if hidden_states.shape[1]!=1:  # prefill stage or training
                    ...
                    # print("before pdrop", type(position_ids))
                    ...
                    stage = self.layer_list.index(rank_layer) # determine current stage
                    # llava-1.5
                    cur_visual_length = [int(sum(self.image_token_ratio_list[stage])) for _ in self.image_tokens]
                    next_visual_length = [int(sum(self.image_token_ratio_list[stage + 1])) for _ in self.image_tokens]
    ...
    def visiondrop_rank_drop(
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            # llava-1.5
            image_tokens = [sum(self.image_token_ratio_list[cur_num]) for _ in self.image_tokens]
            dominant_num = [self.image_token_ratio_list[cur_num + 1][0] for _ in self.image_tokens]
            contextual_num = [self.image_token_ratio_list[cur_num + 1][1] for _ in self.image_tokens]
            keep_length = [d + c for d, c in zip(dominant_num, contextual_num)]
            # image_tokens = [int(round(cur_image_token * self.image_token_ratio_list[cur_num])) for cur_image_token in self.image_tokens]
            # keep_length = [int(round(cur_image_token * self.image_token_ratio_list[cur_num + 1])) for cur_image_token in self.image_tokens]
            ...
            for i in range(batch_size):
                ...
                if self.training:
                    ...
                    image_query_states = cur_query_states[:,image_index:image_index+image_tokens[i],:]
                    image_eager_attention_mask = cur_eager_attention_mask[:,image_index:image_index+image_tokens[i],:]
                
                # w/ causal mask
                # calculate attention map
                attn_weights = torch.matmul(image_query_states, cur_key_states.transpose(1, 2)) / math.sqrt(head_dim) #(num_head, image_token, seq_len)
                attn_weights = attn_weights + image_eager_attention_mask
                attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype) #(num_head, image_token, seq_len)
                ...
                attention_avg_image = torch.mean(attention_avg_head, dim=0) # (576)


                metric = cur_key_states.mean(0)[image_index:image_index+image_tokens[i], :] #(image_token, head_dim)

                vision_tokens = features[i][image_index:image_index + image_tokens[i], :]

                topk_indices = attention_avg_image.topk(dominant_num[i]).indices


                mask = torch.ones_like(vision_tokens[:, 0], dtype=torch.bool, device=metric.device)
                mask[topk_indices] = False

                dominant_tokens = vision_tokens[~mask].view(dominant_num[i], vision_tokens.shape[1])

                ### Filter
                metric_filtered = metric[mask].view(vision_tokens.shape[0] - dominant_num[i], metric.shape[1])

                vision_tokens_filtered = vision_tokens[mask].view(vision_tokens.shape[0] - dominant_num[i], vision_tokens.shape[1])

                metric_normalized = metric_filtered / metric_filtered.norm(dim=-1, keepdim=True)

                ## Contextual Visual Tokens
                step = max(1, metric_normalized.shape[0] // contextual_num[i])
                target_indices = torch.arange(0, metric_normalized.shape[0], step, device=metric_normalized.device)[:contextual_num[i]]
                target_tokens = metric_normalized[target_indices, :]

                tokens_to_merge = metric_normalized[~torch.isin(torch.arange(metric_normalized.shape[0], device=metric_normalized.device),
                                              target_indices), :]
                similarity = torch.matmul(tokens_to_merge, target_tokens.T)
                assign_one_hot = torch.zeros(tokens_to_merge.shape[0], contextual_num[i],
                                             dtype=vision_tokens_filtered.dtype, device=metric_normalized.device)
                assign_one_hot.scatter_(1, similarity.argmax(dim=1, keepdim=True), 1)
                counts = assign_one_hot.sum(dim=0).clamp(min=1).unsqueeze(-1)
                hidden_to_merge = vision_tokens_filtered[~torch.isin(
                    torch.arange(vision_tokens_filtered.shape[0], device=vision_tokens_filtered.device),
                    target_indices), :]
                aggregated_hidden = torch.matmul(assign_one_hot.T, hidden_to_merge) / counts
                target_hidden = vision_tokens_filtered[target_indices, :]

                contextual_tokens = target_hidden + aggregated_hidden

                # Merge with target hidden states and concatenate
                features_save = torch.cat([dominant_tokens, contextual_tokens], dim=0).to(features[i].dtype)


                start_index = image_index + image_tokens[i]
                new_input_embeds = torch.cat(
                    [features[i][:image_index, :], features_save, features[i][start_index:, :]], dim=0)
                combined_indices = torch.cat([topk_indices, target_indices]).sort().values # w/ contextual
                absolute_indices = combined_indices + image_index
                new_labels = torch.cat([labels[i][:image_index], labels[i][absolute_indices], labels[i][start_index:]], dim=0)
                new_attention_mask = torch.cat([attention_mask[i][:image_index], attention_mask[i][absolute_indices], attention_mask[i][start_index:]], dim=0)
